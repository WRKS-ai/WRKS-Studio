    ...
    <section className="py-32 px-6 lg:px-8 border-t border-line">
      <div className="max-w-3xl mx-auto">
        <motion.p
          ...
          className="text-[10px] tracking-[0.22em] uppercase text-ink-muted font-sans mb-5 text-center"
        >
          Questions
        </motion.p>
        <motion.h2
  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
      >
        <span className="font-serif text-lg sm:text-xl text-ink leading-snug">
          {q}
        </span>
        <span
          className={`shrink-0 size-7 rounded-full border border-line flex items-center justify-center text-ink-muted group-hover:border-ink/40 group-hover:text-ink transition-all ${
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <p className="pb-6 pr-12 text-base text-ink-muted leading-relaxed font-sans">
              {a}
            </p>
          </motion.div>
        )}
